...
- [ ] **DB Schema:** Create `crm_roles`, `crm_permissions`, and `crm_role_permissions` tables.
- [ ] **Backend API:** Build API endpoints for Company Admins to manage custom roles.
- [ ] **Backend Logic:** Refactor NestJS Guards to use the new dynamic permission system.
- [ ] **Frontend UI:** Build the "Roles & Permissions" settings page for Company Admins.
...
- [ ] **Backend API:** Create Super Admin endpoints to list, view, and manage tenant organizations.
- [ ] **Backend API:** Implement a user impersonation feature for support purposes.
- [ ] **Frontend UI:** Build a dedicated "Super Admin" section in the application.
- [ ] **Frontend UI:** Develop the tenant list and detail views for the Super Admin.

### 2.2. Global Analytics & Health Monitoring
- [ ] **Backend Instrumentation:** Integrate a metrics solution (e.g., Prometheus) into key backend services.
...
- [ ] **Job Queue:** Integrate BullMQ and Redis to process background jobs (e.g., CSV exports, bulk emails).
- [ ] **Pagination:** Implement API-level pagination (offset or cursor-based) for all list endpoints.
...
- [ ] **Frontend Pagination:** Update all list pages to correctly handle and display paginated data from the API.
